            ...
            <main className="flex-1 overflow-hidden p-6 flex flex-col gap-6 relative z-10">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="px-2 py-0.5 rounded bg-blue-500/10 border border-blue-500/30 text-[9px] font-black text-blue-400 uppercase tracking-widest">
                                {loading ? "Analyzing Evidence..." : "Neural Temporal View"}
                            </span>
                        </div>
                        <p className="text-xs text-slate-500 font-medium tracking-wide italic">
                            {loading 
                                ...
                                : `Visualizing ${events.length} extracted events across the investigation period.`}
                        </p>
                    </div>
                ...
                <div className="flex-1 flex gap-6 min-h-0">
                    ...
                    <div className="flex-1 bg-[#0d1424]/60 border border-[#1e3a5f]/30 rounded-3xl flex flex-col overflow-hidden backdrop-blur-sm relative">
                        {loading && (
                            <div className="absolute inset-0 bg-[#0d1424]/80 backdrop-blur-md z-20 flex flex-col items-center justify-center gap-4">
                                <Loader2 className="w-10 h-10 text-blue-500 animate-spin" />
                                <div className="text-[10px] font-black text-blue-400 uppercase tracking-[0.3em] animate-pulse">Neural Temporal Analysis</div>
                            </div>
                        )}
                        ...
                        <div className="flex-1 relative flex flex-col items-center justify-center p-8">
                            ...
                            <div className="mt-8 text-center max-w-sm">
                                <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Analysis status</div>
                                <p className="text-[11px] text-slate-600 font-medium italic">
                                    {events.length > 0 
                                        ? `Timeline reconstructed from ${currentCaseId ? "Operation Vanguard Omega" : "Active Case"} data streams.`
                                        : "No temporal data points identified in current evidence vault."}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="w-[450px] flex flex-col gap-6">
                        ...
                        <div className="flex-1 bg-[#0d1424]/60 border border-[#1e3a5f]/30 rounded-3xl flex flex-col overflow-hidden backdrop-blur-sm">
                            <div className="px-6 py-4 border-b border-[#1e3a5f]/40 bg-white/[0.02]">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h3 className="text-xs font-black text-white uppercase tracking-widest">Chronological Evidence</h3>
                                </div>
                            </div>
                            ...
                            <div className="p-4 flex-1 overflow-y-auto custom-scrollbar space-y-2">
                                {events.length === 0 && !loading && (
                                    <div className="h-full flex flex-col items-center justify-center text-center p-8 opacity-40">
                                        ...
                                        <p className="text-[10px] text-slate-600 mt-2">Upload more documents to allow AI to reconstruct activity sequences.</p>
                                    </div>
                                )}
                                ...
                                {events.map((event, idx) => (
                                    ...
                                    >
                                        ...
                                        <div className="text-[12px] font-bold text-slate-200 mb-1 group-hover:text-blue-400 transition-colors uppercase tracking-tight">{event.title}</div>
                                        <p className="text-[10px] text-slate-500 leading-relaxed mb-2 line-clamp-2">{event.description}</p>
                                        <div className="flex items-center gap-2">
                                            <span className="text-[9px] font-bold text-slate-600 uppercase">{event.category}</span>
                                            <span className="text-[9px] text-slate-800">•</span>
                                            <span className="text-[9px] font-bold text-slate-600 uppercase">Certainty: {event.confidence}%</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="p-6 bg-blue-500/5 border-t border-[#1e3a5f]/40">
                                <div className="flex items-start gap-4 mb-5">
                                    ...
                                    <div>
                                        <div className="text-xs font-black text-white uppercase tracking-widest mb-1">Temporal Synthesis</div>
                                        <p className="text-[10px] text-slate-500 leading-tight">Sync these events with the global case report and generate timeline visualizations.</p>
                                    </div>
                                </div>
                                <button className="w-full py-3 bg-blue-600 hover:bg-blue-500 rounded-xl text-xs font-black text-white transition-all shadow-lg shadow-blue-500/20 flex items-center justify-center gap-2 uppercase tracking-[0.2em] disabled:opacity-50" disabled={events.length === 0}>
                                    Commit {events.length} Events
                        ...
                        <div className="h-20 bg-[#0d1424] border border-[#1e3a5f]/40 rounded-2xl px-6 flex items-center gap-4">
                            ...
                            <div>
                                <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Temporal Intelligence</div>
                                <div className="text-xs font-bold text-slate-300">
                                    {loading ? "Processing..." : `Synchronized ${new Date().toLocaleTimeString()}`}
                                </div>
                            </div>
                        </div>
                    </div>
