            ...
            <div className="bg-[#111827] border border-slate-800 p-8 rounded-2xl w-full max-w-md shadow-2xl relative overflow-hidden">
                ...
                <div className="flex flex-col items-center mb-8">
                    <div className="w-16 h-16 bg-blue-500/10 rounded-full flex items-center justify-center mb-4 border border-blue-500/20">
                        {view === "verify_otp" ? <MailCheck className="text-blue-500" size={32} /> : <Fingerprint className="text-blue-500" size={32} />}
                    </div>
                    <h1 className="text-2xl font-bold text-white tracking-tight">
                        {view === "verify_otp" ? "Verify Agent Identity" : "Investigator Access"}
                    </h1>
                    <p className="text-slate-400 text-sm mt-1">Nexus Intelligence Platform</p>
                </div>
                ...
                <form onSubmit={handleAuth} className="space-y-4">
                    ...
                    {view === "verify_otp" && (
                        <div>
                            ...
                            />
                            <p className="text-xs text-slate-500 mt-2 text-center">We&apos;ve sent an authentication code to {email}</p>
                        </div>
                    )}
                    ...
                    >
                        {loading ? "Authenticating..." : view === "login" ? <><LogIn size={18} /> Enter Platform</> : view === "verify_otp" ? "Verify Code" : <><UserPlus size={18} /> Register Agent</>}
                    </button>
                </form>
